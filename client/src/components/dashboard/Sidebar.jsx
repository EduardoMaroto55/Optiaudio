...
const Sidebar = (props) => {
    return (
        <div className="relative flex-shrink-0 items-center p-4 min-h-screen bg-slate-800 md:w-60 w-36">
            <Link to="/inicio"><img src={Logo} alt="Logo de empresa" className="h-14 mt-3 object-contain" /></Link>
            ...
            <nav className='mt-5 flex flex-col items-center'>
                <button className="flex hover:font-bold text-white text-xl md:text-2xl cursor-pointer mb-5" onClick={() => props.view('dashboard')}>
                    <BiSolidDashboard size={30} className='text-white mr-2 ' />Dashboard
                </button>
                <button className="flex hover:font-bold text-white text-xl md:text-2xl cursor-pointe" onClick={() => props.view('usuarios')}>
                    <FiUsers size={30} className='text-white mr-2' />Usuarios
                </button>
            </nav>
            <div className="fixed bottom-4 left-10 md:left-24 ">
                <Link to='/login'><BiLogOut className='text-white' size={60} /></Link>
            </div>
        </div>
    )
}
